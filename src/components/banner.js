...
export default (props) =>{
  ...
  const matches = props.matches.slice(0,14)
  return (
    <div style={{width:'90%', margin:"auto", borderBottom:'solid white 1px', marginTop:'1%', paddingBottom:'1%'}}>
    <Swiper
          modules={[Autoplay]}
          spaceBetween={50}
          slidesPerView={5}
        ...
        >
          {matches.map(i=><SwiperSlide>
            <div style={{color:"white", margin:'auto', textAlign:'center'}}>
                {Math.round(i[1])}%  
                <img style={{paddingLeft:'2%'}} src={icons[i[0][0]]}></img>{i[0][0]}
                <img style={{paddingLeft:'5%'}} src={cross}></img>
                {i[0][1]}
                <img style={{paddingLeft:'2%'}} src={icons[i[0][1]]}></img>
                {Math.round(100-i[1])}%</div>
                <div style={{color:"white", textAlign:'center'}}>{`${(i[0][2]).split('-')[1]}/${(i[0][2]).split('-')[2].split(':')[0]} - ${(i[0][2]).split('-')[2].split(':')[1]}:${(i[0][2]).split('-')[2].split(':')[2]}`}</div>
                </SwiperSlide>)}
        </Swiper>
    </div>
