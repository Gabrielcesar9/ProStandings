...
function Home(props){
 ...
 const icons = {'RED':RED, 'PNG':PNG, 'LLL':LLL,'KBM':KBM, 'FUR':FUR, 'RNS':RNS, 'NMG':NMG, 'LBR':LBR, 'ITZ':ITZ,'FLA':FLA,
    ...
    return (
    <>
    <div style={{color:'white', margin:'5%', paddingLeft:'4%', textAlign:'center', fontSize:'50px', fontFamily:"'Space Mono', monospace;"}}>Welcome to ProStandings <img style={{width:'4%', marginBottom:'2%'}} src={plot}></img></div>
    <Navbar/>
    <div style={{border:'dotted white 5px', color:"white",margin:'auto', fontSize:'xx-large', marginTop:'5%', padding:'4%', width:'90%'}}>
        ...
        <p style={{marginTop:'2%'}}>&nbsp;&nbsp;&nbsp;&nbsp;The Glicko-2 rating system is intended to estimate the skill of player/team
         performance in head-to-head competition in a variety of games. It was invented by <a href="http://www.glicko.net/" target="_blank">Mark Glickman</a> as an improvement on the Elo rating system.
         In comparison to the latter, the Glicko-2 rating system presents two significant advantages:</p>
        <br></br>
        <ul>
            <li> It measures volatility/uncertainty;</li>
            <li>Inactivity is ‘penalized’ with an increase in uncertainty;</li>
        </ul>
        <br></br>
        <p>&nbsp;&nbsp;&nbsp;&nbsp;Glicko-2 is used in several esports titles including DotA, CS:GO, being one of the most popular algorithms in the literature used to assess player’s/team’s strengths and predict matches’ outcomes.</p>
        <p>A thorough description of the algorithm is provided by Mark E. Glickman himself and can be found at <a target="_blank" href='http://www.glicko.net/glicko/glicko2.pdf'>http://www.glicko.net/glicko/glicko2.pdf</a>.</p>
        <br></br>
        <p>&nbsp;&nbsp;&nbsp;&nbsp;To those that are new to the Glicko rating system or new to rating systems in general, some FAQs are provided and answered below:</p>
        <ol style={{marginLeft:'2%'}}>
            <br></br>
            <li>How do rating systems work?
                <p>- They work by stablishing and progressively updating players’/teams’ ratings through the application of a predetermined rating algorithm (Glicko-2 is one of many available). After a match between two player’s/team’s is completed, the algorithm updates the competitors’ ratings based on their current ratings and the match outcome.</p>
            </li>
            <br></br>
            <li>Is the Glicko-2 system accurate?
                <p>- In short: yes. However, as with every mathematical model, the Glicko-2 system is only as accurate as the amount of data provided to it and the predictability of the context upon which the algorithm is applied. For instance, Glicko-2 performs well when predicting chess matches outcomes, but poorly when predicting the best player at coin fliping.</p>
            </li>
            <br></br>
            <li>Is it possible for a competitor with a lower win/loss ratio to have a rating than that of another competitor with a higher win/loss ratio?
                <p>- Yes. Rating calculations take into account how ‘strong’ or ‘weak’ the opponent of a certain competitor was when they competed against one another. Hence, it is possible for a competitor to have a higher rating if its losses were to ‘strong’ opponents (with higher ratings) when compared to another competitor whose (fewer) losses were against ‘weaker’ opponents.</p>
            </li>
        </ol>
        <br></br>
        <h1 style={{color:'#E96753'}}>Acknowledgments</h1>
        <br></br>
        <p>&nbsp;&nbsp;&nbsp;&nbsp;Thanks to Mark Glickman for not only devising Glicko system but also improving upon it with Glicko-2. Also, a big thanks to <a href='https://lol.fandom.com/wiki/League_of_Legends_Esports_Wiki' target='_blank'>Leaguepedia</a> for maintaining the amazing database used in the calculations presented here.</p>
    </div>
